...
import { MessageCircle, Layout, MousePointerClick, Sparkles, ListChecks, Mail } from 'lucide-react';
...
export const NewsletterOutput: React.FC<Props> = ({ data, compact }) => {
    if (!data) return null;

    const title = data.title || '';
    const subjectLines = data.subject_lines || [];
    const previewText = data.preview_text || '';
    const newsletterContent = data.newsletter_content || '';
            ...
            {previewText && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-primary)' }}>Preview Text</span>
                    </div>
                    <span style={{ fontSize: 12.5, color: 'var(--text-secondary)', lineHeight: 1.55, fontStyle: 'italic' }}>
                        {previewText}
                    </span>
                </div>
            )}
            ...
            {newsletterContent && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-primary)' }}>Newsletter Content</span>
                    </div>
                    <span style={{
