...
import { Layout, MousePointerClick, Clock, ShoppingCart, Bookmark } from 'lucide-react';
...
export const PinterestAdSocialOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 12 : 16 }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Pinterest — Promoted Pin</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>
                        {posts.length > 0 ? `${posts.length} Ad${posts.length !== 1 ? 's' : ''} Generated` : data.title || 'Pinterest Ad'}
                    </span>
                </div>
            </div>
            ...
            {posts.length > 0 && posts.map((post: any, idx: number) => (
                <div key={idx} style={{
                            ...
                            {post.assets.map((asset: any, ai: number) => (
                                <div key={ai} style={{ overflow: 'hidden', borderBottom: '1px solid var(--border-default)' }}>
                                    {asset.asset_type === 'video' ? (
                                        <video src={asset.gcs_url} controls style={{ width: '100%', display: 'block', maxHeight: 240, objectFit: 'cover' }} />
                                    ) : (
                                        <img src={asset.gcs_url} alt="" style={{ width: '100%', display: 'block', maxHeight: 240, objectFit: 'cover' }} />
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{
                                ...
                                background: 'rgba(230,0,35,0.1)', border: '1px solid rgba(230,0,35,0.2)',
                    ...
                    <div style={{ padding: 14 }}>
                        ...
                        {/* Shopping intent fields */}
                        {(post.price || post.product_category || post.shopping_url) && (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                                {post.price && (
                                    <span style={{
                                        ...
                                        <ShoppingCart size={12} /> {post.price}
                                    </span>
                                )}
