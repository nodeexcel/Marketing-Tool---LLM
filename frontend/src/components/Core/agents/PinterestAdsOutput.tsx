...
import { Megaphone, FileText, MousePointerClick, Layout, ImageIcon, Bookmark, Sparkles, ListChecks } from 'lucide-react';
...
export const PinterestAdsOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 12 : 16 }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Pinterest Ads</span>
            ...
            {variations.map((v: any, i: number) => (
                <div key={i} style={{
                    ...
                    <div style={{ padding: compact ? 12 : 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                        {(v.pin_title || v.headline) && (
                            <div style={{ display: 'flex', gap: 10 }}>
                                <Layout size={16} color="var(--text-muted)" style={{ marginTop: 2, flexShrink: 0 }} />
                                <div>
                                    <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, marginBottom: 2 }}>PIN TITLE</div>
                        ...
                        {(v.description || v.body) && (
                            <div style={{ display: 'flex', gap: 10 }}>
                                ...
                                <div>
                                    <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, marginBottom: 2 }}>DESCRIPTION</div>
                        ...
                        {v.board_suggestion && (
                            <div style={{ display: 'flex', gap: 10 }}>
                                <Bookmark size={16} color="var(--text-muted)" style={{ marginTop: 2, flexShrink: 0 }} />
                                <div>
                                    <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, marginBottom: 2 }}>BOARD SUGGESTION</div>
                                    <div style={{
