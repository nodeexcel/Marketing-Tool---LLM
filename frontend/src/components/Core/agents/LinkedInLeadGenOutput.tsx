...
import { Megaphone, FileText, MousePointerClick, Layout, Users, ClipboardList, Sparkles, ListChecks } from 'lucide-react';
...
export const LinkedInLeadGenOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 12 : 16 }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>LinkedIn Lead Gen Ads</span>
                    {data?.title && <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-primary)' }}>{data.title}</span>}
                </div>
            </div>

            {variations.map((v: any, i: number) => {
                const formFields = v.form_fields || [];
                        ...
                        <div style={{ padding: compact ? 12 : 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                            ...
                            {(v.intro_text || v.body) && (
                                <div style={{ display: 'flex', gap: 10 }}>
                                    ...
                                    <div>
                                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, marginBottom: 2 }}>INTRO TEXT</div>
                            ...
                            {formFields.length > 0 && (
                                <div style={{ display: 'flex', gap: 10 }}>
                                    <ClipboardList size={16} color="var(--text-muted)" style={{ marginTop: 2, flexShrink: 0 }} />
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, marginBottom: 6 }}>FORM FIELDS</div>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                            {formFields.map((field: string, idx: number) => (
                                                <div key={idx} style={{
                                                    padding: '6px 12px',
