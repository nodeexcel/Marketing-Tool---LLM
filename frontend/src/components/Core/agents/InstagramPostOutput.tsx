...
import { Instagram, Clock, Hash, Image, Video, Eye } from 'lucide-react';
...
export const InstagramPostOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    {imageCount > 0 && (
                        <span style={{
                            ...
                            <Image size={12} /> {imageCount} Image{imageCount !== 1 ? 's' : ''}
                        </span>
                    )}
                    {videoCount > 0 && (
                        <span style={{
                            ...
                            <Video size={12} /> {videoCount} Video{videoCount !== 1 ? 's' : ''}
                        </span>
                    )}
                </div>
            </div>
                                    ...
                                    {post.assets.map((asset: any, ai: number) => (
                                        <div key={ai} style={{ borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border-default)', position: 'relative' }}>
                                            ...
                                            <span style={{
                                                ...
                                                background: asset.asset_type === 'video' ? 'rgba(193,53,132,0.85)' : 'rgba(225,48,108,0.85)',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    {sections.map((s: any, i: number) => (
                        <div key={i} style={{
                            ...
                            {s.content && (
                                <span style={{ fontSize: 12.5, color: 'var(--text-secondary)', lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>{s.content}</span>
                            )}
                        </div>
                    ))}
                </div>
