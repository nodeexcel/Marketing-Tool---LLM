...
import { Megaphone, FileText, MousePointerClick, Layout, Facebook, Instagram, Tag, Sparkles, ListChecks } from 'lucide-react';
...
export const MetaAdsOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 12 : 16 }}>
            ...
            <div style={{
                ...
                background: 'linear-gradient(135deg, rgba(59,130,246,0.12), rgba(124,92,255,0.06))',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Meta Ads</span>
                    {data?.title && <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-primary)' }}>{data.title}</span>}
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                    <Facebook size={14} color="var(--text-muted)" />
                    <Instagram size={14} color="var(--text-muted)" />
                </div>
            </div>
            ...
            {/* Fallback: sections when no variations */}
            ...
            {/* Fallback: recommendations + action_items */}
