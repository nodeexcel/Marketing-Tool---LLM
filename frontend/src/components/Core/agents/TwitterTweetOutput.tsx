...
export const TwitterTweetOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 12 : 16 }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Twitter / X — Tweet</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>
                        {items.length > 0 ? `${items.length} Post${items.length !== 1 ? 's' : ''} Generated` : data.title || 'Tweet'}
                    </span>
                </div>
            </div>

            {/* Tweet cards */}
            {items.length > 0 && items.map((post: any, idx: number) => {
