...
import { Video, Clock, Music, Zap } from 'lucide-react';
...
export const TikTokScriptOutput: React.FC<Props> = ({ data, compact = false }) => {
    if (!data) return null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 12 : 16 }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>TikTok — Script</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>
                        {posts.length > 0 ? `${posts.length} Script${posts.length !== 1 ? 's' : ''} Generated` : data.title || 'TikTok Script'}
                    </span>
                </div>
            </div>

            {/* Script cards — video-centric layout */}
            {posts.length > 0 && posts.map((post: any, idx: number) => (
                <div key={idx} style={{
                    ...
                    <div style={{ padding: 14 }}>
                        ...
                        {/* Video prompts */}
                        {post.video_prompts?.length > 0 && (
                            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
                                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Video Prompts</span>
                                {post.video_prompts.map((p: string, pi: number) => (
